...
class MemoryMaintenanceScheduler:
    def __init__(self, worker: LifecycleWorker, *, tenant_id: str, authority: object,
                 ...
                 promotion_callback: Callable[[float], object] | None = None,
        ...
        if promotion_callback is not None and not callable(promotion_callback):
            raise ValueError("maintenance promotion callback must be callable")
    ...
    def run_once(self) -> None:
        ...
        for cleanup in self._cleanup_callbacks:
            cleanup(epoch)
        # Promotion is deliberately a separate host-owned phase.  It runs
        # only after lifecycle cleanup has applied the current forgetting
        # policy, and receives the same maintenance window timestamp so the
        # evaluator can re-check expiry and provenance at execution time.
        if self._promotion_callback is not None:
            self._promotion_callback(epoch)
