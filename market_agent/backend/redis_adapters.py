...
from market_agent.backend.errors import DependencyUnavailableError, TaskQueueFullError
...
class RedisPromptActivationMirror:
    ...
    def __call__(self, activation: Any, pin: Any) -> object:
        trace_id = sha256(
            f"prompt:{activation.active_release_id}:{activation.action}:{activation.revision}".encode()
        ).hexdigest()[:32]
        self._bus.publish(MessageEnvelope(
...
class RedisStreamMessageBus:
    ...
    def recover_owned_pending(self, *, topic: str, group: str, consumer: str,
                              count: int = 1) -> tuple[StreamDelivery, ...]:
        if not all(type(value) is str and value.strip() for value in (topic, group, consumer)) or count < 1:
            raise ValueError("stream consume arguments are invalid")
        stream = self._stream(topic)
        try:
            # ID 0 reads only this consumer's PEL, without waiting for the
            # cross-owner idle threshold or blocking for new deliveries.
            batches = self._client.xreadgroup(group, consumer, {stream: "0"}, count=count)
            return tuple(self._deliveries(stream, batches))
        except RedisUnavailableError:
            raise
        except Exception as error:
            raise RedisUnavailableError("Redis stream owned pending recovery failed") from error
...
class RedisMessageBusAdapter:
    ...
    def subscribe(self, topic: str, handler: Any):
        ...
        def consume() -> None:
            failures = 0
            pending_cursor = "0-0"
            unresolved_deliveries: set[str] = set()
            try:
                while not self._stop.is_set() and not local_stop.is_set():
                    active_delivery: StreamDelivery | None = None
                    try:
                        pending = self._bus.recover_owned_pending(
                            topic=topic,
                            group=self._group,
                            consumer=consumer,
                        ...
                        transient_delivery_failure = False
                        for delivery in deliveries:
                            ...
                            try:
                                ...
                                if self._is_transient_delivery_error(error):
                                    unresolved_deliveries.add(delivery.message_id)
                                    failures += 1
                                    self._mark_consumer_degraded(health_key, error)
                                    if self._wait_for_retry(local_stop, self._retry_delay(failures)):
                                        return
                                    transient_delivery_failure = True
                                    break
                            ...
                            # An empty poll or another delivery's ACK cannot
                            # resolve this ID; only ACK or completed DLQ+ACK can.
                            unresolved_deliveries.discard(delivery.message_id)
                            active_delivery = None
                        if transient_delivery_failure:
                            continue
                    except RedisUnavailableError as error:
                        if active_delivery is not None:
                            unresolved_deliveries.add(active_delivery.message_id)
                        failures += 1
                        self._mark_consumer_degraded(health_key, error)
                        if self._wait_for_retry(local_stop, self._retry_delay(failures)):
                            return
    ...
    def _is_transient_delivery_error(error: Exception) -> bool:
        return (
            isinstance(error, (TaskQueueFullError, DependencyUnavailableError, RedisUnavailableError))
