...
from market_agent.workflow_evaluation import EvaluationBinding, EvaluationExecutor, EvaluationRun, EvaluationRunner
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--execution-mode", choices=("recorded", "executed"), default="recorded")
    parser.add_argument("--executor-factory", help="Explicit module:factory for the current workflow evaluator")
    return parser


def _load_executor_factory(reference: str) -> Callable[[], EvaluationExecutor]:
    module_name, separator, attribute = reference.partition(":")
    if not separator or not module_name or not attribute.isidentifier():
        raise ValueError("executor factory must be an explicit module:factory reference")
    factory = getattr(import_module(module_name), attribute)
    if not callable(factory):
        raise ValueError("executor factory is not callable")
    return factory


def run(argv: Sequence[str] | None = None, *,
        executor_factory: Callable[[], EvaluationExecutor] | None = None) -> tuple[int, dict[str, object]]:
    ...
    try:
        ...
        if args.execution_mode == "executed":
            if executor_factory is not None and args.executor_factory is not None:
                raise ValueError("executor factory must be supplied through exactly one channel")
            factory = executor_factory or (_load_executor_factory(args.executor_factory) if args.executor_factory else None)
            if factory is None:
                raise ValueError("executed evaluation requires an explicit workflow executor factory")
            executor = factory()
            candidate = runner.run(dataset, binding=binding, execution_mode="executed", executor=executor)
        else:
            if executor_factory is not None or args.executor_factory is not None:
                raise ValueError("recorded evaluation cannot configure a workflow executor")
            candidate = runner.run(dataset, binding=binding, execution_mode="recorded")
        baseline = None
