...
from market_agent.workflow_model_budget import pricing_band_for_cost_admission
...
def rendered_request_token_upper_bound(
    ...
    """Return a deterministic conservative count for legacy consumers.

    The cost-admission path uses its own serialized wire representation plus
    framing allowance. This helper remains for callers that need the earlier
    canonical-message bound.
    """
...
def pricing_band_for_rendered_request(
    ...
    return pricing_band_for_cost_admission(messages, output_schema_json)
...
class CacheRequest:
    ...
    # Host embedding work is a paid pre-model attempt, even when its vector is
    # unusable. The owner observes that attempt before returning this context.
    embedding_cost_usd: float = 0.0

    def __post_init__(self) -> None:
        if (type(self.embedding_cost_usd) not in (int, float)
                or not math.isfinite(self.embedding_cost_usd)
                or self.embedding_cost_usd < 0):
            raise ValueError("cache embedding cost must be finite and nonnegative")
...
class CacheUsageObservationError(RuntimeError):
    """A paid cache vector attempt cannot be charged to the host ledger."""
...
class _Run:
    ...
    # A semantic cache context may require a paid host embedding. Reuse the
    # same immutable context for lookup and accepted safe-write admission.
    cache_context_computed: bool = False
    cached_cache_context: CacheRequest | None = None
...
class AgentDriver:
    ...
    def execute(self, invocation: AgentInvocation, *, prompt_pin: PromptPin | WorkflowPromptPin | None = None,
        ...
        except CacheUsageObservationError:
            return self._failure(invocation.trace_id, "usage_observation_unavailable")
    ...
    def _store_safe_response(self, run: _Run, schema: OutputSchema,
        ...
        try:
            ...
            if self._exact is not None:
                ...
                self._emit(run, "fixed_cache_write", "completed", outcome="completed")
            if self._semantic is not None and context.vector is not None:
                ...
                self._emit(run, "semantic_cache_write", "completed", outcome="completed")
        except (_AuditFailure, CacheUsageObservationError):
            raise
        except Exception:
    ...
    def _bound_cache_context(self, run: _Run) -> CacheRequest | None:
        if self._cache_context is None:
            return None
        if not run.cache_context_computed:
            run.cache_context_computed = True
            ...
            if context is not None:
                ...
                run.spent += Decimal(str(context.embedding_cost_usd))
            run.cached_cache_context = context
        context = run.cached_cache_context
        if context is None:
            return None
        if not isinstance(context, CacheRequest):
            raise TypeError("cache context must be a typed request")
    ...
    def _model(self, run: _Run, schema: OutputSchema) -> AgentResult | None:
        ...
        while True:
            ...
            try:
                ...
                dispatch_now = self._clock.now()
                if dispatch_now >= invocation.deadline_epoch:
                    if circuit.kind == "probe":
                        self._record_circuit(run, success=False, probe=True)
                    return None
                request = replace(
                    request,
                    deadline_epoch=min(
                        invocation.deadline_epoch,
                        dispatch_now + invocation.attempt_timeout_seconds,
                    ),
                )
                run.attempts += 1
                per_tier_attempts += 1
