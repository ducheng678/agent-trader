...
def _inferred_core_work_item(
    ...
    *,
    ...
    execution_state: str,
    recovery_dependency_ids: tuple[str, ...] = (),
    ...
    No arbitrary provider task is admitted: only fixed core model tasks and
    the closed host embedding registry can create such an entry.
    """

    if verified_embedding_work_item(task_id, node, attempts):
        return ObservedWorkItem(
            task_id=task_id,
            task_kind="embedding",
            worker_id="embedding-agent",
            owner_node=node,
            dependency_ids=recovery_dependency_ids if node is CoreNodeName.RECOVER else (),
            maximum_retries=0,
...
def _observed_node(
    ...
    # Keep the observation boundary authoritative even when a node action
    # contains an unexpected exception.  Most node implementations already
    # fail closed themselves, but assembly/risk and host-provided callbacks
    # may still raise.  Converting that exception into the same safe result
    # lets the observer append exactly one terminal checkpoint and gives the
    # Harness a chance to reject/stop the run before the graph unwinds.
    try:
        update = action(state)
        if not isinstance(update, dict):
            raise TypeError("workflow node action must return a mapping")
    except Exception:
        update = _safe_failure(state, f"{node.value} execution is unavailable")
    ...
    attempts_for_task: dict[str, list[AttemptUsage]] = {}
    ...
    for attempt in attempts:
        ...
        attempts_for_task.setdefault(attempt.task_id, []).append(attempt)
    ...
    embedding_task_ids = {
        task_id for task_id in inferred_task_ids
        if verified_embedding_work_item(
            task_id, first_node_by_task[task_id], tuple(attempts_for_task[task_id]),
        )
    }
    ...
    for task_id in inferred_task_ids:
        ...
        if task_id in embedding_task_ids and owner in {
            CoreNodeName.PLAN, CoreNodeName.DISPATCH, CoreNodeName.RECOVER,
        }:
            if attempts_for_task[task_id][0].source == "embedding_response":
                completed_set.add(task_id)
            else:
                failed_set.add(task_id)
        elif owner is CoreNodeName.DECIDE and node is not CoreNodeName.PLAN:
    ...
    inferred_work_items = tuple(
        ...
        if (item := _inferred_core_work_item(
            ...
            attempts=tuple(attempts_for_task[task_id]),
            execution_state=(
                "succeeded" if task_id in completed else
                "failed" if task_id in failed else "running"
            ),
            recovery_dependency_ids=(
                tuple(task.parent_task_id for task in tasks
                      if task.parent_task_id in prior_task_ids)[:1]
                or prior_task_ids[:1]
            ) if first_node_by_task[task_id] is CoreNodeName.RECOVER else (),
        )) is not None
    )
