...
from market_agent.workflow_embedding_work_item import verified_embedding_work_item
...
class HarnessKernel:
    ...
    def _validate_checkpoint_sequence(
        ...
        for checkpoint in checkpoints:
            ...
            for task_id in checkpoint.task_ids:
                ...
                registered_worker = (
                    "embedding-agent" if item.task_kind == "embedding"
                    else "reflection-agent" if item.task_kind == "reflection"
                    else f"{item.task_kind}-agent"
                )
                try:
                    if item.task_kind not in {"reflection", "embedding"}:
                ...
                if item.task_kind == "embedding":
                    if (item.maximum_retries != 0
                            or not verified_embedding_work_item(
                                task_id, item.owner_node, task_attempts,
                            )):
                        raise HarnessDependencyError("embedding work item has unverified host provenance")
                    completed = task_attempts[0].source == "embedding_response"
                    if (item.execution_state != ("succeeded" if completed else "failed")
                            or (task_id in checkpoint.completed_task_ids) != completed
                            or (task_id in checkpoint.failed_task_ids) == completed):
                        raise HarnessDependencyError("embedding work-item progress is inconsistent")
                elif any(attempt.source in {
                    "embedding_response", "embedding_usage_unavailable",
                } for attempt in task_attempts):
                    raise HarnessDependencyError("embedding cannot borrow a specialist task identity")
    ...
    @staticmethod
    def _explicit_checkpoint_free_usage(usage: WorkflowUsage) -> bool:
        """Authorize only the two host-owned non-graph result routes.

        Graph specialists still require durable node checkpoints. A cache lookup
        may have paid for an embedding, and a static sourced answer may have a
        bounded informational model attempt; both are charged by the same
        durable budget projection below.
        """

        if not usage.attempts or any(item.node is not CoreNodeName.PLAN for item in usage.attempts):
            return False
        embeddings = {"embedding_response", "embedding_usage_unavailable"}
        cached = {"historical_cache", *embeddings}
        if (sum(item.source == "historical_cache" for item in usage.attempts) == 1
                and all(item.source in cached for item in usage.attempts)):
            return all(
                (item.task_id == "historical-cache" if item.source == "historical_cache"
                 else item.task_id == "historical-query")
                for item in usage.attempts
            )
        informational = {
            "provider_response", "provider_usage_unavailable",
            "fixed_cache", "semantic_cache", "local_knowledge", "abstention",
        }
        return (
            any(item.task_id == "informational" for item in usage.attempts)
            and all(
                ((item.task_id == "historical-query" and item.source in embeddings)
                 or (item.task_id == "informational" and item.source in informational))
                for item in usage.attempts
            )
        )
    ...
    def _policy_decision(
        ...
        if workflow_usage is not None:
            ...
            if (
                ...
                and not self._explicit_checkpoint_free_usage(checked_usage)
            ):
                raise HarnessDependencyError(
