...
class MemoryQuery(MemoryContract):
    ...
    max_candidates_scanned: Annotated[PositiveInt, Field(le=10000)] = 1024
...
class VectorCandidatePage(MemoryContract):
    records: Annotated[tuple[Record, ...], Field(max_length=1024)] = ()
    ...
    def validate_progress(self):
        if self.exhausted and self.next_cursor is not None:
            raise ValueError("an exhausted candidate page cannot have a continuation")
        if not self.exhausted and (self.next_cursor is None or not self.records):
            raise ValueError("an incomplete candidate page must make bounded progress")
        return self
...
_VECTOR_PAGE_SIZE = 64
_MAX_EVIDENCE_RECORDS = 8192


class _RetrievalScanLimit(RuntimeError):
    def __init__(self, omission: str):
        super().__init__(omission)
...
def _record_references(record: Record) -> tuple[str, ...]:
    ...
    Vector stores page memory candidates separately from evidence. Evidence and
...
def _retrieval_records(
    ...
    """Exhaust bounded vector pages, then hydrate their evidence ancestry.

    ``vector_candidate_page`` is an optional capability;
    ...
    candidate_page = getattr(repository, "vector_candidate_page", None)
    if query.embedding and callable(candidate_page):
        ...
        scanned = 0
        while True:
            limit = min(_VECTOR_PAGE_SIZE, query.max_candidates_scanned - scanned)
            if limit <= 0:
                raise _RetrievalScanLimit("candidate_scan_limit")
            page = VectorCandidatePage.model_validate(candidate_page(query, cursor=cursor, limit=limit))
            if len(page.records) > limit:
                raise MemoryIntegrityError("candidate page exceeded its requested limit")
            scanned += len(page.records)
            for record in page.records:
                if record.tenant_id != query.tenant_id or record.record_id in candidate_ids:
                    raise MemoryIntegrityError("candidate page has a foreign or repeated identity")
            ...
            if page.next_cursor in cursors:
                raise MemoryIntegrityError("candidate page cursor did not advance")
        ...
        while pending:
            ...
            for identifier in _record_references(current):
                if identifier in seen:
                    continue
                if len(seen) >= _MAX_EVIDENCE_RECORDS:
                    raise _RetrievalScanLimit("evidence_scan_limit")
                seen.add(identifier)
                related = repository.get_by_id(identifier, tenant_id=query.tenant_id)
                if related is None:
                    continue
                if related.tenant_id != query.tenant_id or related.record_id != identifier:
                    raise MemoryIntegrityError("hydrated evidence has the wrong identity")
                records[related.record_id] = related
                pending.append(related)
        return candidates, records

    if query.embedding and callable(getattr(repository, "vector_candidates", None)):
        raise MemoryIntegrityError("vector repositories must report exhaustive candidate pages")
...
def retrieve_memory(query: MemoryQuery, repository: MemoryRepository) -> RetrievalResult:
    ...
    except _RetrievalScanLimit as error:
        return RetrievalResult(**base, status="failed", omissions=(error.omission,))
