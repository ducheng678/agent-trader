...
EmbeddingSource = Literal["embedding_response", "embedding_usage_unavailable"]
UsageSource = ProviderSource | NonProviderSource | EmbeddingSource
...
class AttemptUsage(ObservationModel):
    ...
    pricing_model_id: Literal[
        "gpt-5.6-luna", "gpt-5.6-terra", "gpt-5.6-sol", "text-embedding-3-small"
    ] | None = None
    ...
    def validate_source_shape(self) -> AttemptUsage:
        if self.source in {"embedding_response", "embedding_usage_unavailable"}:
            if (self.provider != "openai" or self.model_tier is not None
                    or self.model_id != EMBEDDING_MODEL_ID
                    or self.pricing_model_id != EMBEDDING_MODEL_ID
                    or self.pricing_version != EMBEDDING_PRICING_VERSION
                    or self.pricing_band is not None):
                raise ValueError("embedding attempts require their exact pinned pricing identity")
            if self.source == "embedding_usage_unavailable":
                if self.tokens is not None or self.estimated_cost_usd <= 0:
                    raise ValueError("unverified embeddings require positive reserved cost and no fabricated tokens")
            else:
                if self.tokens is None or any((
                    self.tokens.output_tokens, self.tokens.cached_input_tokens,
                    self.tokens.cache_write_tokens, self.tokens.web_search_tool_calls,
                )):
                    raise ValueError("embedding responses require verified input-only token usage")
                if Decimal(str(self.estimated_cost_usd)) != embedding_cost(self.tokens.input_tokens):
                    raise ValueError("embedding response cost does not match exact pinned pricing")
            return self
    ...
    def is_provider_attempt(self) -> bool:
        return self.source in {
            "provider_response", "provider_usage_unavailable",
            "embedding_response", "embedding_usage_unavailable",
        }
...
def _usage_values(attempts: tuple[AttemptUsage, ...]) -> dict[str, object]:
    ...
    return {
        ...
        "unverified_provider_attempt_count": sum(
            item.source in {"provider_usage_unavailable", "embedding_usage_unavailable"}
...
class WorkflowExecution(ObservationModel):
    ...
    completion_kind: Literal["graph", "historical_cache", "informational"] = "graph"
    ...
    def validate_execution(self) -> WorkflowExecution:
        ...
        if self.completion_kind == "historical_cache" and (
            self.checkpoints or any(
                item.source not in {"historical_cache", "embedding_response", "embedding_usage_unavailable"}
                for item in self.usage.attempts
            )
        ):
            raise ValueError("historical cache execution permits only embedding and historical cache attempts")
        if self.completion_kind == "informational":
            if self.checkpoints or not self.usage.attempts:
                raise ValueError("direct informational execution requires actual attempts and no graph checkpoints")
            if self.result.final_action is not Action.NO_TRADE or self.result.playbook_payload is not None:
                raise ValueError("direct informational execution cannot authorize trading")
            if self.result.terminal_mode is TerminalMode.INFORMATIONAL:
                answer = self.result.informational_answer
                if (self.result.knowledge_status is not KnowledgeStatus.KNOWN
                        or answer is None or answer.knowledge_status is not KnowledgeStatus.KNOWN
                        or not answer.source_references
                        or not set(answer.source_references) <= set(self.result.evidence_references)):
                    raise ValueError("direct informational answers require known cited evidence")
            elif (self.result.terminal_mode not in {TerminalMode.UNKNOWN, TerminalMode.NO_TRADE}
                  or self.result.knowledge_status is not KnowledgeStatus.INSUFFICIENT):
                raise ValueError("direct informational failure requires an explicit safe abstention")
        return self
