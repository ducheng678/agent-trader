...
from market_agent.backend.prompt_activation_store import (
    PendingPromptActivationAudit,
    PromptActivationState,
    PromptActivationStore,
...
class PromptReleaseManager:
    ...
    def __init__(
        ...
        *,
        ...
        activation_store: PromptActivationStore | None = None,
    ) -> None:
        if not manifests:
    ...
    def current(self) -> PromptPin:
        if self._activation_store is not None:
            ...
            return self._pin_shared_pointer(state.active)
    ...
    @property
    def activation_store(self) -> PromptActivationStore | None:
        """Return the configured authority so production readiness can verify wiring."""
    ...
    def activate(self, release_id: str) -> PromptActivation:
        ...
        if self._activation_store is not None:
            return self._activate_shared(pin)
        with self._lock, self._connection() as connection:
    ...
    def replay_pending_audit(self, *, limit: int = 100) -> int:
        ...
        if self._audit_hook is None:
            return 0
        if self._activation_store is not None:
            return self._replay_shared_pending_audit(limit)
        with self._lock, self._connection() as connection:
    ...
    @staticmethod
    def _pointer(pin: PromptPin) -> PromptReleasePointer:
    ...
    def _pin_shared_pointer(self, pointer: PromptReleasePointer) -> PromptPin:
        if type(pointer) is not PromptReleasePointer:
            raise PromptConfigurationError("shared prompt activation state is malformed")
        manifest = self._manifests.get(pointer.release_id)
        if manifest is None:
            raise PromptConfigurationError("shared prompt release manifest is unavailable locally")
        pin = self._pin(pointer.release_id)
        if pin.release_digest != pointer.release_digest:
            raise PromptConfigurationError("shared prompt release digest does not match local manifest")
        if pin.manifest_hash != pointer.manifest_hash:
            raise PromptConfigurationError("shared prompt manifest hash does not match local manifest")
        return pin

    def _read_shared_state(self) -> PromptActivationState | None:
        assert self._activation_store is not None
        try:
            state = self._activation_store.read()
        except Exception as error:
            raise PromptConfigurationError("shared prompt activation store is unavailable") from error
        if state is not None and type(state) is not PromptActivationState:
            raise PromptConfigurationError("shared prompt activation state is malformed")
        return state

    def _activate_shared(self, pin: PromptPin) -> PromptActivation:
        assert self._activation_store is not None
        pointer = self._pointer(pin)
        with self._lock:
            state = self._read_shared_state()
            if state is None:
                try:
                    changed = self._activation_store.bootstrap(pointer)
                except Exception as error:
                    raise PromptConfigurationError("shared prompt activation store is unavailable") from error
                if type(changed) is not PromptActivationState or changed.active != pointer:
                    raise PromptConfigurationError("shared prompt activation changed concurrently")
                previous_release_id = None
            else:
                self._pin_shared_pointer(state.active)
                if state.active == pointer:
                    return PromptActivation(
                        active_release_id=pin.release_id,
                        previous_release_id=state.active.release_id,
                        action="activate_noop",
                    )
                try:
                    changed = self._activation_store.compare_and_swap(
                        state.revision,
                        pointer,
                        state.active,
                        "activate",
                ...
                if changed is None:
                    raise PromptConfigurationError("shared prompt activation changed concurrently")
                expected = PromptActivationState(
                    active=pointer,
            ...
            self._record_local_audit(activation, pin)
        self._emit_shared(activation, pin)
        return activation

    def _rollback_shared(self) -> PromptActivation:
        assert self._activation_store is not None
        with self._lock:
            state = self._read_shared_state()
            if state is None or state.previous is None:
                raise PromptConfigurationError("no previous prompt release is available for rollback")
            self._pin_shared_pointer(state.active)
            pin = self._pin_shared_pointer(state.previous)
            self._require_gate(pin, "rollback")
            try:
                changed = self._activation_store.compare_and_swap(
                    state.revision,
            ...
            if changed != expected:
                raise PromptConfigurationError("shared prompt activation state is malformed")
            activation = PromptActivation(
                active_release_id=state.previous.release_id,
                previous_release_id=state.active.release_id,
                action="rollback",
                revision=changed.revision,
            )
    ...
    def _replay_shared_pending_audit(self, limit: int) -> int:
        assert self._activation_store is not None
        try:
            records = self._activation_store.pending_audits(limit=limit)
        except Exception as error:
            raise PromptConfigurationError("shared prompt activation store is unavailable") from error
        delivered = 0
        for record in records:
            if type(record) is not PendingPromptActivationAudit:
                raise PromptConfigurationError("shared prompt activation audit is malformed")
            pin = self._pin_shared_pointer(record.active)
            activation = PromptActivation(
                active_release_id=record.active.release_id,
                previous_release_id=(
                    None if record.previous is None else record.previous.release_id
                ),
                action=record.action,
                revision=record.revision,
            )
            try:
                assert self._audit_hook is not None
                self._audit_hook(activation, pin)
            except Exception:
                continue
            try:
                acknowledged = self._activation_store.acknowledge_audit(record.revision)
            except Exception:
                continue
            delivered += int(acknowledged)
        return delivered

    def _record_local_audit(self, activation: PromptActivation, pin: PromptPin) -> None:
        with self._connection() as connection:
            connection.execute("BEGIN IMMEDIATE")
            self._record_activation(connection, activation, pin)
            connection.commit()

    def _emit_shared(self, activation: PromptActivation, pin: PromptPin) -> None:
        """Deliver the shared outbox event using its immutable revision identity."""

        assert self._activation_store is not None
        assert activation.revision is not None
        if self._audit_hook is not None:
            try:
                ...
                try:
                    self._activation_store.acknowledge_audit(activation.revision)
                except Exception:
                    pass
        self._emit_metric(activation, pin)
    ...
    def _emit_metric(self, activation: PromptActivation, pin: PromptPin) -> None:
...
def default_prompt_manager(*, registry_path: Path, git_root: Path | None = None,
                           ...
                           audit_hook: ReleaseHook | None = None,
                           metric_hook: ReleaseHook | None = None,
                           activation_store: PromptActivationStore | None = None) -> PromptReleaseManager:
