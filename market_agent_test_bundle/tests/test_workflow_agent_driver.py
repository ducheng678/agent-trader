...
def test_cache_context_is_computed_once_for_lookup_and_safe_write():
    approved = {schema().digest: {"known"}}
    semantic = SemanticRequestCache(safe_answers=approved)
    calls = []

    def context(inv):
        calls.append(inv.task_id)
        return cache_request()

    driver, _, _ = make_driver(
        Client(response()), semantic_cache=semantic, cache_context=context,
    ...
    assert result.failure is None
    assert calls == ["task-1"]
    assert semantic.lookup((1.0, 0.0), metadata(), 1.0) is not None


def test_paid_cache_embedding_reduces_model_call_allowance():
    client = Client(response())
    driver, _, _ = make_driver(
        client, cache_context=lambda inv: api().CacheRequest(
            metadata=metadata(), vector=None, embedding_cost_usd=0.02,
        ),
    )
    result = driver.execute(invocation(cost_limit_usd=0.11))
    assert client.requests == []
    assert result.output == {"answer": "不知道"}


def test_cache_embedding_ledger_failure_is_terminal_before_model_dispatch():
    client = Client(response())

    def context(inv):
        raise api().CacheUsageObservationError("embedding usage ledger unavailable")

    driver, _, _ = make_driver(client, cache_context=context)
    result = driver.execute(invocation())
    assert client.requests == []
...
def test_cache_embedding_cost_requires_finite_nonnegative_amount(cost):
    with pytest.raises((TypeError, ValueError)):
        api().CacheRequest(metadata=metadata(), embedding_cost_usd=cost)
