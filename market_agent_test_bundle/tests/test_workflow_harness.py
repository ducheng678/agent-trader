...
def test_checkpoint_free_historical_hit_charges_verified_embedding(tmp_path):
    kernel, store, _, _ = _kernel(tmp_path)
    handle = kernel.create(_request())
    _advance_to_running(kernel, handle.run_id)
    observations = ExecutionObservationCollector(handle.run_id, handle.trace_id)
    observations.record_attempt(AttemptUsage(
        ...
        task_id="historical-query", attempt=0, node=CoreNodeName.PLAN,
        provider="openai", provider_request_id="embedding-hit", model_id="text-embedding-3-small",
        ...
        estimated_cost_usd=0.000002, latency_ms=1, source="embedding_response",
    ))
    observations.record_attempt(AttemptUsage(
        workflow_id=handle.run_id, trace_id=handle.trace_id,
        task_id="historical-cache", attempt=0, node=CoreNodeName.PLAN,
        provider="host", provider_request_id="historical-cache-entry-1", model_id="historical-cache",
        model_tier=None, pricing_version="host-zero-v1", tokens=TokenUsage.zero(),
        estimated_cost_usd=0.0, latency_ms=0, source="historical_cache",
    ))
    kernel.advance(handle.run_id, candidate={}, workflow_usage=observations.usage())
    authority = next(event for event in reversed(store.load(handle.run_id))
                     if event.event_type == "transition_authorized" and "budget_settlement_evidence" in event.payload)
    assert authority.payload["budget_delta_attempts"] == 1
    assert authority.payload["budget_delta_tokens"] == 100
    assert authority.payload["budget_delta_cost"] == "0.000002"


def test_checkpoint_free_sourced_informational_usage_is_budgeted_but_other_llm_rejected(tmp_path):
    kernel, store, _, _ = _kernel(tmp_path)
    handle = kernel.create(_request())
    _advance_to_running(kernel, handle.run_id)
    observations = ExecutionObservationCollector(handle.run_id, handle.trace_id)
    attempt = AttemptUsage(
        ...
        provider="openai", provider_request_id="informational-response-1", model_id="gpt-5.6-terra",
        model_tier="terra", pricing_version="openai-standard-2026-08-01",
        pricing_model_id="gpt-5.6-terra", pricing_band="short",
        tokens=TokenUsage(input_tokens=3, output_tokens=2),
        estimated_cost_usd=0.00003, latency_ms=1, source="provider_response",
    )
    observations.record_attempt(attempt)
    kernel.advance(handle.run_id, candidate={}, workflow_usage=observations.usage())
    authority = next(event for event in reversed(store.load(handle.run_id))
                     if event.event_type == "transition_authorized" and "budget_settlement_evidence" in event.payload)
    assert authority.payload["budget_delta_attempts"] == 1
    assert authority.payload["budget_delta_tokens"] == 5
    assert authority.payload["budget_delta_cost"] == "0.00003"

    other_path = tmp_path / "other"
    other_path.mkdir()
    other, _, _, _ = _kernel(other_path)
    other_handle = other.create(_request())
    _advance_to_running(other, other_handle.run_id)
    unrelated = ExecutionObservationCollector(other_handle.run_id, other_handle.trace_id)
    unrelated.record_attempt(attempt.model_copy(update={
        "workflow_id": other_handle.run_id, "trace_id": other_handle.trace_id,
        "task_id": "decision_planner",
    }))
    with pytest.raises(HarnessDependencyError, match="checkpoint"):
        other.advance(other_handle.run_id, candidate={}, workflow_usage=unrelated.usage())
